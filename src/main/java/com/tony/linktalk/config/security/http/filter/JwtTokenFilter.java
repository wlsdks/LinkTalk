...
/**
 * JWT 토큰 필터링 클래스
 * 이 필터에서는 HTTP 요청을 가로채서 JWT 토큰의 유효성을 검사하고, 유효한 경우 사용자 인증 정보를 설정한다.
 * 만약 JWT 토큰 처리 중 오류가 발생하면 JwtAuthenticationException을 던지도록 되어 있다.
 * JwtAuthenticationException은 Spring Security의 AuthenticationException을 상속하므로, 직접 선언하고 등록한 AuthenticationEntryPoint에서 처리된다.
 */
@Slf4j
@Component
